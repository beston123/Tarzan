CREATE TABLE `tevent_message_aggregate_plan` (
...
CREATE TABLE `tevent_to_check_message` (
...
CREATE TABLE `tevent_to_send_message` (
  `tid` bigint(20) NOT NULL COMMENT '事务消息Id',
  `mq_type` tinyint(4) NOT NULL COMMENT 'MQ类型',
  `source_time` datetime NOT NULL COMMENT '消息来源时间',
...
##################################################################
## RocketMQ 消息存储
## tevent_message_rocketmq_{3位分表序号}
##################################################################

CREATE TABLE `tevent_message_rocketmq_000` (
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='RocketMQ消息表';
